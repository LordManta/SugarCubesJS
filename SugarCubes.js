...
function NO_FUN(){};
/*
On définit une constante pour la liste de valeurs d'événment vide.
*/
const VOID_VALUES= [];
Object.freeze(VOID_VALUES);
/*
On définit un identifiant incrémental pour les événements
*/
var nextEventID= 0;
/*
...
les parametres SC sont :
  - une NES (champs d'un Cube)
  - soit un entier
  - un boolean (true ou false)
  - une fonction
  - un objet binding avec t objet et f function ou NES.
...
binding n'est pas encore définit, on retourne l'objet SC_LateBinding en fixant
l'objet cube sur lequel il porte pour réaliser une late binding ou un binding
dynamique.
*/
function _b(cube){
    return function(o){
...
/*
Fonction de gestion des paramètres numériques
*/
...
/*
Fonction de vérification des paramètres de type fonction
*/
function isFun(fun){
    if(undefined==fun){
      return false;
...
function PurgeableCollection(){
  ...
  this.collection= (undefined!=sc_global.WeakMap)
                   ?(new sc_global.WeakMap()):{};
  this._sc_weak= undefined!=sc_global.WeakRef;
  this.allKeys= [];
  Object.freeze(this);
  };
(function(proto){
proto.get= (undefined!=sc_global.WeakMap)
  ? function(sym){
      return this.collection.get(sym);
      }
  : function(sym){
      const tk= this.collection[sym];
      return (this._sc_weak && tk)?tk.deref():tk;
      };
proto.has= (undefined!=sc_global.WeakMap)
  ? function(sym){
      return this.collection.has(sym);
      }
  : function(sym){
      return undefined!==this.get(sym);
      };
proto.set= (undefined!=sc_global.WeakMap)
  ? function(sym, val){
      if(undefined===val){
...
proto.rm= (undefined!=sc_global.WeakMap)
  ? function(sym){
      const idx= this.allKeys.indexOf(sym);
      if(idx>=0){
        this.allKeys.splice(idx, 1);
        }
      return this.collection.delete(sym, val);
      }
  : function(sym){
      const idx= this.allKeys.indexOf(sym);
      if(idx>=0){
...
proto.cleanup= (undefined!=sc_global.WeakMap)
  ? NO_FUN
  : function(){
...
d'ordre sémantique... On va à nouveau dissocier machine et sensor.
  ...
  3. la valeur du représentant d'un sensor est unique et a une seule valeur à
     chaque réaction (échantillonage).
*/
const SC_Runtime= {
...
function SC_LateBinding(){
  ...
  // on ne retourne pas la valeur de l'expression pour s'assurer du type
  // booléen.
  Object.defineProperty(this, "vf"
  , { value: (args && args.vf)?true:false , writable: false });
  if(testNES(obj)){// Si obj est une chaîne c'est le nom sur cube
    Object.defineProperty(this, "name", { value: obj , writable: false });
    }
  else if(testTA(obj)){ // si c'est un target action
    Object.defineProperty(this, "tgt"
    , { value: obj.t, writable: false });
    Object.defineProperty(this, "name"
    , { value: obj.f, writable: false });
    }
  else if("function"==typeof(obj)){ // si c'est une fonction on tient
                                    // compte de vf
...
/*********
SC_CubeExposedState (interface object)
**********/
/*
Allows to expose part of a cube state in event generation.
*/
...
/*********
SC_CubeExposedStateInternal (implementation object)
**********/
 ...
 *  - ajout d'un comportement en parallèle (les programmes sont émis sur
...
function SC_Event(id, m){
  ...
  // instant.
  this.vals= [];
  // Observer la génération permanante qui pose problème...
...
/*
Gestion des abonnements d'instructions.
*/
...
*/
  ...
  else{ /* Gestion des liens aux événements du DOM. */
          ...
          if(b.hasPotential){
            if(undefined != b.itsParent){
              if(!b.itsParent.hasPotential){
                throw "Here we are, invalid registration of potential emiters";
                }
              }
            if(b.idxInProd<0){
...
*/
proto.registerForProduction= function(b/*, perma*/){
        ...
        if(undefined!==el){
          ...
          for(var n= 0; n<selen; n++){
            ...
            if(isEvent(see)){
              ...
              throw new Error("Bad side Effect specification: "+se);
              }
            }
          }
...
proto.bindTo= function(engine, parbranch, seq, path, cube, cinst){
    if(engine instanceof SC_Machine){
      const binder= _b(cube);
      const tmp_evt= binder((null===this.evt)?SC_WRITE_EVT:this.evt).bindTo(engine);
...
/*********
 * SC_SimpleActionOnEventNoDef Class
 *********/
...
On ne veut pas exposer bindTo() à l'extérieur or ici c'est le cas...
*/
      Object.defineProperty(this, "bindTo", {
          value: function(engine){
...
function SC_Cube(o, p, extension){
  ...
  if(extension){
    ...
    if(extension.swapList){
      //console.warn("swapList", extension.swapList);
...
proto.bindTo= function(engine, parbranch, seq, path, cube, cinst){
    if(engine instanceof SC_Machine){
      ...
      if(cells){
        const clen= cells.length;
	//console.warn("cube state", clen);
        for(var n= 0;n<clen; n++){
          const k= cells[n];
          switch(k.type){
            case 'fun':{
	      //console.warn("fun", copy.o[k.id]);
              copy.exposedState[k.id] = copy.o[k.id].bind(copy.exposedState);
              break;
              }
            case 'const':{
	      //console.warn("const", copy.o[k.id]);
              copy.exposedState[k.id] = copy.o[k.id];
              break;
              }
            case 'var':
            default:{
	      //console.warn("var", copy.o[k.id]);
              swap_text+= "  this."+k.id
                        + " = state."+k.id
                        + ";";
              break;
 ...
 *********/
var nextMachineID= 0;
const SC_WRITE_EVT= new SC_EventId({ name: "SC_WRITE_EVT" });
const SC_WRITE_ID= new SC_EventId({ name: "SC_WRITE_ID" });
const SC_DUMP_ID= new SC_EventId({ name: "SC_DUMP_ID" });
...
*/
    ...
    if(this.Evt_dump.isPresent(this)){
      const keys= this.environment.getKeys();
...
*/
  ...
  Object.defineProperty(SC, "dumpE"
  , { value: function(){
          return new SC_GenerateOneNoVal(SC_DUMP_ID);
          }
    , writable: false
      }
    );
  ...
  , { value: function(count){
        ...
        else if('object'==typeof(count)
             && "object"==typeof(count.t) && testNES(count.f)){
            return new SC_Next({ f: count.f, t: count.t });
          }
        else if(isNaN(count) || count<=0 || count>1000000){
          throw new Error(
...
/* --- Cubes --- */
  ...
  , { value: function(o, prg, extensions){
          if(undefined==extensions || "object"!=typeof(extensions)){
            extensions= {};
            }
          if(undefined==o || "object"!=typeof(o)){
            throw new Error("Invalid object for cube: "+o);
            }
          extensions.root= o;
          if(undefined==prg || !prg.isAnSCProgram){
            throw new Error("undefined program for cube: "+prg);
            }
          extensions.prg= prg;
  ...
  , { value: function(params){
          ...
          /*
          Conservé pour compatibilité mais actions devrait être le bon moyen de
          faire.
          */
          const funs= params.methods;
          if(funs && Array.isArray(funs)){
            const flen= funs.length;
          ...
          if(meths && "object"==typeof(meths)){
            ...
            for(var n= 0; n<mlen; n++){
              ...
              p.root[met]= meths[met];
              }
            }
          /*
          compatibilité avec l'ancienne version de l'API
          */
          ...
          if(cells){
            ...
            for(var n= 0; n<clen; n++){
              const i= cells[n];
              if(!testNES(i.id) || (undefined!==i.type && "string"!=typeof(i.type))){
                throw new Error("cubify state name "+i.id+" not valid "
                                +"with type "+i.type+"in "+cells);
                }
              switch(i.type){
                case 'fun':{
                  if(typeof(p.root[i.id]) != "function"){
              ...
              p.life.swapList.push(i);
              }
            }
          if("object"==typeof(params.life)){
            const life= params.life;
            if(undefined!==life.init && "function"!=typeof(life.init)){
              throw new Error("invalid params.life.init");
              }
            if(undefined!==life.lastWill && "function"!=typeof(life.lastWill)){
              throw new Error("invalid params.life.lastWill");
              }
	  ...
	  //console.warn("calling new SC_Cube() with", p, p.life);
  ...
  Object.defineProperty(SC, "toCell"
                          , { value: function(params){
                            // tgt, nom, fun, el, sub
    const p= {};
    if("object"!=typeof(params) || "object"!=typeof(params.target)
      || !testNES(params.name)|| (params.el && !Array.isArray(params.el))){
    ...
    p.el= params.el;
    var t= p.target;
    if(Array.isArray(params.sub)){ // sous champs...
      const sublen= params.sub.length;
      p.sub= [];
      for(var n= 0; n<sublen; n++){
        const nd= params.sub[n];
        if(!testNES(nd)){
          throw new Error("Invalid subname fiels: '"+nd+"'");
          }
        p.sub.push(nd);
        t= t[nd];
        if(undefined===t){
          throw new Error("Sub field doesn't seem to exist: '"+params.sub+"'");
          }
        }
      }
    else if(testNES(params.sub)){
      t= p.target[params.sub];
      t.sub= params.sub;
      }
    if(undefined===t){
      throw new Error("Field doesn't seem to exist: "+params.sub+"'")
      }
    p.store= ("object"==typeof(params.store))?params.store:p.target;
    const funn="_scc_"+p.name;
    if('function'==typeof(params.fun)){
      p.store[funn]= params.fun.bind(p.target);
      }
...
*/
  ...
  , { value: function(tgt, name, init, el, fun){
        ...
        if(undefined==fun && undefined==tgt[fun_name]){
          throw new Error("undefined affectator on "+tgt+" or "+arguments);
          }
  ...
  , { value: function(p){// {?target: filed:  sideEffect: ?init:}
          ...
          const params= {};
          if(undefined!==p.target && 'object'!=typeof(p.target)){
            throw new Error("undefined target of cell: "+p.target);
            }
          if(undefined!==p.field && !testNES(p.field)){
            throw new Error("undefined target name of cell: "+p.field);
            }
          if(p.sideEffect && 'function'!=typeof(p.sideEffect)){
            throw new Error("Bad side Effect specification: "+p.sideEffect);
            }
          ...
          if(p.eventList){
            ...
            const selen= se.length;
            for(var n= 0; n<selen; n++){
              const see= p.eventList[n];
              if(isEvent(see)){
                params.eventList.push(see);
                }
              else{
                throw new Error("Bad side Effect specification: "+p.eventList);
                }
              }
            }
...
const initConfigParser= new RegExp(
    `^${sns}*{${sns}*` // Overture objet
  +`${first_entry}${sns}*`
  +`(${following_entries}${sns}*)*`
  + `}${sns}*$`
  );
/*
On distingue le loader node et le loader navigateur...
*/
if('undefined'==typeof(window)){
  console.warn("In node");
  Object.defineProperty(SC, "loader"
  , { value: new SC_Loader(global), writable: false });
  // On construit les fonctions de chargement pour node
  //const suf= ["", "_min", "_stat"].includes(global.sc_suffix)
  //            ?(""+global.sc_suffix):"_min";
  //const path= (global.sc_path && "string"==typeof(global.sc_path)
  //               && /^([^\s,\n\r\\]|\\.)*$/.test(global.sc_path))
  //            ?""+global.sc_path:"./";
  const fs= require('fs');
  function read(f) {
  ...
  //SC.loader.include({ src: `${path}SugarCubes${suf}.js` });
  global.SC= SC;
  module.exports= { SC };
  ...
  if(initConfigParser.test(paramSrc)){
    ...
    // Si les paramètres ne sont pas encore utilisés on applique l'ancien mode...
    Object.defineProperty(SC, "init"
    , { value: function(p= {}){
          if(undefined!==SC.globals.init_params){
            throw new Error("Fatal Error invalid init parameters... "+paramSrc);
